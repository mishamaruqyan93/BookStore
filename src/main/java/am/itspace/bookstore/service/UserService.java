...
public class UserService {
    ...
    public void save(User user) throws DuplicateResourceException, MessagingException {
        ...
        mailService.sendHtmEmail(user.getEmail(), "Please verify your email", "Hi" + user.getName()
                + "\n" + "Please verify your account by clicking on this link" +
                " <a href= \"http://localhost:8081/user/verify?email=" + user.getEmail() + "&token=" + user.getVerifyToken() + "\">Activate</a>");
    }
    ...
    public void verifyUser(String email, String token) throws Exception {
        ...
        if (userOptional.isEmpty()) {
            throw new Exception("User does not exist email and token");
        }
        User user = userOptional.get();
        if (user.isEnable()) {
            throw new Exception("User already enable");
        }
        user.setEnable(true);
